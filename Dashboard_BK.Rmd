...
```{r}
data <- read.csv("~//Documents/STUDY/DATA_SCIENCE/DataCamp_data/Interactive_dashboard/VehicleFailure.csv")
```
...
p2 <- data %>%
         ...
         add_pie(hole = 0.2) %>%     #  mojno izmenit chtob umenshit vnutrenniy pustoy krug
...
```{r}
car <- data %>% group_by(State) %>% summarize(total = n())
...
Created by: Data Scientist Irina Max
